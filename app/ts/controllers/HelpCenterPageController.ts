...
export class HelpCenterPageController {
    ...
    constructor(currentPage: number = 1, totalPages: number = 1) {
        ...
        this.questionView.didMount(() => {
            Array.from(document.querySelectorAll('a.can-resolve')).forEach(button => {
                const id = button.getAttribute('resolve-help')
                button.addEventListener('click', this.resolved.bind(this, id))
            })
        })
    ...
    resolved(id: string, event: Event) {
        event.preventDefault();
        document.getElementById('resolve-help').textContent = 'Resolvido'
        document.getElementById('resolve-help').setAttribute('desabled', 'desabled')
    }
